...
/**
 * Utility: Safely extract relative file path for DB storage
 */
function getFilePath(file) {
    if (!file) return undefined;
    const normalizedPath = file.path.replace(/\\/g, '/');
    return normalizedPath.includes("public/")
    ...
    return (req, res) => {
        ...
        upload(req, res, async (err) => {
            ...
            try {
                ...
                // ✅ Extract file paths
                const photo = getFilePath(files.photo?.[0]);
                const aadhar_file = getFilePath(files.aadhar_file?.[0]);
                const pan_file = getFilePath(files.pan_file?.[0]);
                const dl_file = getFilePath(files.dl_file?.[0]);
                ...
                res.status(201).send({ status: true, data: "Client Created Successfully", client });
            } catch (error) {
...
exports.get = async (req, res) => {
    ...
    return await ClientSchema.aggregate([
        ...
        res.status(200).send({
            status: true,
            datas,
            pagination: { total, totalPages, limit },
        });
    });
};
    ...
    return (req, res) => {
        upload(req, res, async (err) => {
            ...
            try {
                ...
                await ClientSchema.findByIdAndUpdate(req.params.id, body);
...
exports.filter = async (req, res) => {
    ...
    if (type === "active") params = { ...params, is_active: true };
    else params = { ...params, is_active: false };
    ...
    if (from_date && to_date) {
        params = {
            ...
            createdAt: {
                ...
                $lt: new Date(moment(to_date).add(1, "d")),
            },
        };
    }

    const limit = 25;
    const total = await ClientSchema.find(params).count();
...
exports.upload_excel = (upload, multer) => {
    return (req, res) => {
        ...
        upload(req, res, async (err) => {
            ...
            const read = excelReader.readExcel(req.file, headers);

            if (!read) return res.status(422).json(read);
            ...
            for (const item of read) {
                ...
                try {
                    await ClientSchema.create(createData);
                } catch (err) {
                    errors.push({
            ...
            errors.length
                ? res.status(500).json({ status: true, msg: errors })
