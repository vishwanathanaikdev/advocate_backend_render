...
exports.userLogin = (req, res) => {
  User.findOne({
    ...
    .exec((err, data) => {
      ...
      if (!data) {
        return res.status(401).json({ status: false, errors: "Invalid User Info" });
      }

      // ✅ compare hashed passwords
      if (bcrypt.compareSync(req.body.password, data.password))
        {
        return res.status(401).json({ status: false, errors: "Invalid Password" });
      }
      ...
      UserRole.find({ user_id: payload.id })
        .populate({ path: "role_id", model: "Role", select: "role_name" })
        .exec(async (err, roleData) => {
          if (err) return res.status(500).json({ status: false, errors: err.errors });

          payload.roles = roleData?.length ? roleData.map((r) => r?.role_id?.role_name) : [];
          payload.token = jwt.createToken(payload);
          payload.status = true;
